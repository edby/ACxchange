        ...
        $(function () {
            ...
            var opt = {
                ...
                onPageClicked: function (event, originalEvent, type, newPage) {
                    ...
                    $.ajax({
                        url: "{{route('withdrawal_check','reject')}}",
                            ...
                            if (msg.code == 200) {
                                ...
                                for (var i in msg[0]['data']) {
                                    str += `<tr trid="${msg[0]['data'][i]['id']}" user_id="${msg[0]['data'][i]['user_id']}" amount="${msg[0]['data'][i]['amount']} ${msg[0]['data'][i]['currency']}">
                                                ...
                                                <td>${msg[0]['data'][i]['currency']} ${msg[0]['data'][i]['fee']}</td>
            ...
            $(".audit_export").on('click',function () {
                window.location.href = ('{{env('ADMIN_DOMAIN').'/withdrawal/reject'}}?&t_start='+filter[0]+'&t_end='+filter[1]+'&currency_id='+filter[2]+'&user_id='+filter[3]+'&export=withdraw');
